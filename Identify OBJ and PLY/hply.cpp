...
/* 2026新增: 插件模式错误码定义(替代直接exit终止进程), 见 hply.h 说明 */
int HPLY::ply_error = 0;

#ifdef _WIN32
#include <windows.h>   /* plyFOpen() 需要宽字符路径转换, 支持中文路径 */
#endif

/* 2026新增: Windows下UTF-8路径转宽字符打开文件, 解决中文路径 fopen 失败问题 */
static FILE* plyFOpen(const char* path, const char* mode)
{
#ifdef _WIN32
  wchar_t wpath[1024];
  wchar_t wmode[16];
...
void HPLY::ascii_get_element(HPLY::PlyFile *plyfile, char *elem_ptr)
{
  ...
  if (words == NULL) {
    ...
    /* 2026修改: 原为 exit(-1) 终止进程, 插件模式下改为设置错误码并返回 */
    ply_error = 2;
    return;
  }
...
void HPLY::binary_get_element(HPLY::PlyFile *plyfile, char *elem_ptr)
{
  ...
  for (j = 0; j < elem->nprops; j++) {
    ...
    if (prop->is_list) {       /* a list */
      ...
      /* allocate space for an array of items and store a ptr to the array */
      list_count = int_val;
      /* 2026新增: 列表长度异常保护(文件损坏时防止错误的大块内存分配) */
      if (list_count < 0 || list_count > 100000000) { ply_error = 1; return; }
      item_size = ply_type_size[prop->internal_type];
      store_array = (char **) (elem_data + prop->offset);
      if (list_count == 0) {
        if (store_it)
...
unsigned char has_x, has_y, has_z;
  ...
  float version;
	PlyVertex vert_temp;
  	...
  	FILE *p_out=fopen(out_v_file,"w+b"); //�����vertex�ļ�
	FILE *p_out_a=fopen("vertex.b.asc ","w+t"); //�����vertex�ļ�,asc
	FILE *fp_f=fopen(out_f_file,"w+b");//�����face�ļ�
  ...
  for (i = 0; i < nelems; i++) 
  {
    ...
    if (equal_strings ("vertex", elem_name))
	{
	  ...
	  for (j = 0; j < num_elems; j++) 
	  {
		...
		s=smallest( (temp_f.verts), ply, fp_v);//����������ָ���ҵ����㣬����С�ĺ�����
...
{
	FILE *in_f=plyFOpen(file, "rb");
	if (!in_f)
    {
          fprintf(stderr, "Error:  Couldn't open file, %s, for reading\n", file);
          /* 2026修改: 原为 exit(7) 终止进程, 插件模式下改为设置错误码并返回 */
          ply_error = 3;
          nvf[0]=-1;
          nvf[1]=-1;
          return;
	...
	/* 2026新增: 头解析失败保护, 原代码 ply 为 NULL 时直接解引用崩溃 */
	if (ply == NULL)
	{
		ply_error = 4;
		fclose(in_f);
