...
#include <assert.h>
#ifdef _WIN32
#include <windows.h>   /* glmFOpen() 需要宽字符路径转换, 支持中文路径 */
#endif
#include "glm.h"

/* ============================================================
   修改说明(2026)：为能在 Unity 插件中使用，做了以下最小修改：
   1. 新增 glmFOpen(): Windows 下 UTF-8 路径转宽字符打开文件，
      解决中文路径 fopen 失败问题; 其他平台仍用 fopen。
   2. glmReadMTL 中的 AfxMessageBox(MFC) 改为 fprintf 输出。
   3. glmReadOBJ 打开文件失败时返回 NULL(原来 exit(1) 会终止进程)。
   4. glmDraw/glmList 仅在具备 OpenGL 环境时编译(渲染由 Unity 负责)。
   ============================================================ */
static FILE* glmFOpen(const char* path, const char* mode)
{
#ifdef _WIN32
  wchar_t wpath[1024];
  wchar_t wmode[16];
...
static char* glmDirName(char* path)
{
  ...
  /* 2026修改: 原代码只识别'\\'分隔符, 在macOS('/')下会取错目录导致MTL打不开 */
  s = strrchr(dir, '\\');
  s2 = strrchr(dir, '/');
  if (s2 && (!s || s2 > s))
    s = s2;
...
static GLvoid glmReadMTL(GLMmodel* model, char* name)
{
  ...
  file = glmFOpen(filename, "r");
  if (!file)
  {
    /* 修改(2026): 原为 AfxMessageBox(MFC依赖), 改为标准错误输出 */
...
GLMmodel* glmReadOBJ(char* filename)
{
  ...
  file = glmFOpen(filename, "r");
  if (!file) {
    /* 修改(2026): 原为 exit(1) 会直接终止进程, 插件模式下改为返回 NULL 由调用者处理 */
 ...
 */
#ifndef NO_OPENGL_RENDER   /* 修改(2026): 无OpenGL环境(Unity插件)下不编译渲染代码 */
